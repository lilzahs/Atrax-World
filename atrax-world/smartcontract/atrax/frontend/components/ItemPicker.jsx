...
export default function ItemPicker({
  ...
  async function onChoose() {
    try {
      ...
      const sig = await chooseItem({ connection, wallet, programId, roomId: id, itemType, streamer, devWallet });
      setMsg(`Thành công: ${sig}`);
      onSuccess?.({ kind: 'item', roomId: id, itemType, priceSol });
    } catch (e) {
  ...
  return (
    <div className="card">
      ...
      <div className="row" style={{ gap: 8, marginTop: 8 }}>
        {ITEMS.map(i => (
          <button key={i} className={`btn ${itemType===i?'primary':''}`} onClick={() => setItemType(i)}>#{i}</button>
        ))}
      </div>
      ...
      <div className="row" style={{ marginTop: 10 }}>
        <button className="btn primary" disabled={busy} onClick={onChoose}>{busy ? 'Đang xử lý…' : 'Chọn ngay'}</button>
      </div>
